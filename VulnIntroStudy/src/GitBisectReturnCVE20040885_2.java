...
/**
 * CVE-2004-0885: /modules/ssl/ssl_engine_kernel.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20040885_2 {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("int ssl_hook_Access(request_rec *r)")>0
					&&stringBuffer.indexOf("if (sk_SSL_CIPHER_find(cipher_list, cipher) < 0) {")>0
					&&stringBuffer.indexOf("SSL_CIPHER_get_name(cipher));")>0
					&&stringBuffer.indexOf("if (cipher_list) {")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("int ssl_hook_Access(request_rec *r)")>0
					&&stringBuffer.indexOf("if (sk_SSL_CIPHER_find(cipher_list, cipher) < 0) {")<0
					&&stringBuffer.indexOf("SSL_CIPHER_get_name(cipher));")<0
					&&stringBuffer.indexOf("if (cipher_list) {")<0
					) {
