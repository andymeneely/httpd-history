...
/**
 * CVE-2004-0113: /modules/ssl/ssl_engine_io.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20040113 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2004-0113>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("static void ssl_io_filter_disable(ap_filter_t *f)")>0
					&&stringBuffer.indexOf(" bio_filter_in_ctx_t *inctx = f->ctx;")>0
					&&stringBuffer.indexOf("SSL_free(inctx->ssl);")>0
					&&stringBuffer.indexOf("inctx->ssl = NULL;")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("static void ssl_io_filter_disable(ap_filter_t *f)")>0
					&&stringBuffer.indexOf(" bio_filter_in_ctx_t *inctx = f->ctx;")>0
					&&stringBuffer.indexOf("SSL_free(inctx->ssl);")<0
					&&stringBuffer.indexOf("inctx->ssl = NULL;")>0
					) {
