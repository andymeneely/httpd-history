...
/**
 * CVE-2009-1195: /modules/filters/mod_include.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20091195_2 {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("if ((ap_allow_options(r) & OPT_INC_WITH_EXEC) == 0) {")>0
					&&stringBuffer.indexOf("ctx->flags |= SSI_FLAG_NO_EXEC;")>0
					&&stringBuffer.indexOf("if (ap_allow_options(r) & OPT_INCNOEXEC) {")<0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("if (ap_allow_options(r) & OPT_INCNOEXEC) {")>0
					&&stringBuffer.indexOf("ctx->flags |= SSI_FLAG_NO_EXEC;")>0
					) {
