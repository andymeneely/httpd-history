...
/**
 * CVE-2010-0434: /modules/http/http_request.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20100434_1 {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("new->headers_in      = apr_table_copy(r->pool, r->headers_in);")>0
					&&stringBuffer.indexOf("r->headers_in = apr_table_overlay(r->pool, rr->headers_in,")>0
					&&stringBuffer.indexOf("static request_rec *internal_internal_redirect(const char *new_uri,")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("new->headers_in      = r->headers_in;")>0
					&&stringBuffer.indexOf("r->headers_in = apr_table_overlay(r->pool, rr->headers_in,")<0
					&&stringBuffer.indexOf("static request_rec *internal_internal_redirect(const char *new_uri,")>0
					) {
