...
/**
 * CVE-2009-3094: /modules/proxy/mod_proxy_ftp.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20093094 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2009-3094>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("static apr_port_t parse_epsv_reply(char *reply)")>0
					&&stringBuffer.indexOf("if (errno || port < 1 || port > 65535 || ep[0] != p[1] || ep[1] != ')') {")>0
					&&stringBuffer.indexOf("data_port = parse_epsv_reply(ftpmessage);")>0
					&&stringBuffer.indexOf("if (data_port) {")>0
					&&stringBuffer.indexOf("data_port = atoi(pstr + 3);")<0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("if (pstr) {")>0
					&&stringBuffer.indexOf("data_port = atoi(pstr + 3);")>0
					&&stringBuffer.indexOf("static apr_port_t parse_epsv_reply(char *reply)")<0
					) {
