 ...
 * 
 */
public class GitBisectReturnCVE20040885_1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2004-0885>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("static void ssl_init_ctx_protocol(server_rec *s,")>0
					&&stringBuffer.indexOf("#ifdef SSL_OP_NO_SESSION_RESUMPTION_ON_RENEGOTIATION")>0
					&&stringBuffer.indexOf("SSL_CTX_set_options(ctx, SSL_OP_NO_SESSION_RESUMPTION_ON_RENEGOTIATION);")>0
					&&stringBuffer.indexOf("* Disallow a session from being resumed during a renegotiation,")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("static void ssl_init_ctx_protocol(server_rec *s,")>0
					&&stringBuffer.indexOf("#ifdef SSL_OP_NO_SESSION_RESUMPTION_ON_RENEGOTIATION")<0
					&&stringBuffer.indexOf("SSL_CTX_set_options(ctx, SSL_OP_NO_SESSION_RESUMPTION_ON_RENEGOTIATION);")<0
					&&stringBuffer.indexOf("* Disallow a session from being resumed during a renegotiation,")<0
					) {
