...
/**
 * CVE-2005-3352: /modules/mappers/mod_imagemap.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20053352_1 {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("static char *imap_url(request_rec *r, const char *base, const char *value)")>0
					&&stringBuffer.indexOf("if (referer && *referer) {")>0
					&&stringBuffer.indexOf("return ap_escape_html(r->pool, referer);")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("static char *imap_url(request_rec *r, const char *base, const char *value)")>0
					&&stringBuffer.indexOf("if (referer && *referer) {")>0
					&&stringBuffer.indexOf("return apr_pstrdup(r->pool, referer);")>0
					) {
