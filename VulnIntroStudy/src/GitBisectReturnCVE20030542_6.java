 ...
 * 
 */
public class GitBisectReturnCVE20030542_6 {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("#define LS_REG_PATTERN \"^ *([0-9]+) +([^ ]+)$\"")>0
					&&stringBuffer.indexOf("#define LS_REG_MATCH   3")>0
					&&stringBuffer.indexOf("regmatch_t re_result[LS_REG_MATCH];")>0
					&&stringBuffer.indexOf("re = ap_pregcomp(p, LS_REG_PATTERN, REG_EXTENDED);")>0
					&&stringBuffer.indexOf("else if (0 == ap_regexec(re, ctx->buffer, LS_REG_MATCH, re_result, 0)) {")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("regmatch_t re_result[3];")>0
					&&stringBuffer.indexOf("re = ap_pregcomp(p, \"^ *([0-9]+) +([^ ]+)$\", REG_EXTENDED);")>0
					&&stringBuffer.indexOf("else if (0 == ap_regexec(re, ctx->buffer, 3, re_result, 0)) {")>0
					) {
