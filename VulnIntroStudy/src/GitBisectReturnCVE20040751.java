...
/**
 * CVE-2004-0751: /modules/ssl/ssl_engine_io.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20040751 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2004-0751>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("static apr_status_t ssl_io_input_read(bio_filter_in_ctx_t *inctx,")>0
					&&stringBuffer.indexOf("if (inctx->mode == AP_MODE_SPECULATIVE) {")>0
					&&stringBuffer.indexOf("if (inctx->cbuf.length > 0) {")>0
					&&stringBuffer.indexOf("char_buffer_write(&inctx->cbuf, buf, (int)bytes);")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("static apr_status_t ssl_io_input_read(bio_filter_in_ctx_t *inctx,")>0
					&&stringBuffer.indexOf("if (inctx->mode == AP_MODE_SPECULATIVE) {")>0
					&&stringBuffer.indexOf("if (inctx->cbuf.length > 0) {")<0
					&&stringBuffer.indexOf("char_buffer_write(&inctx->cbuf, buf, (int)bytes);")<0
					) {
