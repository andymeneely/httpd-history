...
/**
 * CVE-2005-2970: /server/mpm/worker/worker.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20052970 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2005-2970>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("apr_pool_t *ptrans = NULL;            /* Pool for per-transaction stuff */")>0
					&&stringBuffer.indexOf("apr_pool_t *recycled_pool = NULL;")<0
					&&stringBuffer.indexOf("if (ptrans == NULL) {")>0
					&&stringBuffer.indexOf("ptrans = recycled_pool;")<0
					&&stringBuffer.indexOf("recycled_pool = ptrans;")<0
					&&stringBuffer.indexOf("ap_scoreboard_image->servers[process_slot][thread_slot].tid = apr_os_thread_current();")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("apr_pool_t *ptrans;                /* Pool for per-transaction stuff */")>0
					&&stringBuffer.indexOf("apr_pool_t *recycled_pool = NULL;")>0
					&&stringBuffer.indexOf("if (recycled_pool == NULL) {")>0
					&&stringBuffer.indexOf("ptrans = recycled_pool;")>0
					&&stringBuffer.indexOf("recycled_pool = ptrans;")>0
					) {
