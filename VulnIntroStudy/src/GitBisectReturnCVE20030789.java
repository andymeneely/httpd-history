 ...
 * 
 */
public class GitBisectReturnCVE20030789 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2003-0789>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("apr_os_pipe_put_ex(&tempsock, &sd, 1, r->pool);")>0
					&&stringBuffer.indexOf("apr_pool_cleanup_kill(r->pool, (void *)sd, close_unix_socket);")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("apr_os_pipe_put(&tempsock, &sd, r->pool);")>0
					&&stringBuffer.indexOf("apr_pool_cleanup_kill(r->pool, (void *)sd, close_unix_socket);")>0
					) {
