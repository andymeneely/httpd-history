...
/**
 * CVE-2005-2700: /modules/ssl/ssl_engine_kernel.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20052700 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2005-2700>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("if (!renegotiate && verify != verify_old) {")>0
					&&stringBuffer.indexOf("(!(verify_old & SSL_VERIFY_FAIL_IF_NO_PEER_CERT) &&")>0
					&&stringBuffer.indexOf("(verify     & SSL_VERIFY_FAIL_IF_NO_PEER_CERT)))")>0
					&&stringBuffer.indexOf("renegotiate = TRUE;")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("if (!renegotiate && verify != verify_old) {")>0
					&&stringBuffer.indexOf("(!(verify_old & SSL_VERIFY_PEER_STRICT) &&")>0
					&&stringBuffer.indexOf("(verify     & SSL_VERIFY_PEER_STRICT)))")>0
					&&stringBuffer.indexOf("renegotiate = TRUE;")>0
					) {
