...
/**
 * CVE-2010-1452: /modules/session/mod_session.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20101452_3 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2010-1452>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf(" if (conf->includes->nelts) {")>0
					&&stringBuffer.indexOf("if (strncmp(r->uri, include, strlen(include))) {")>0
					&&stringBuffer.indexOf("if (strncmp(r->uri, exclude, strlen(exclude))) {")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("if (conf->includes->nelts) {")>0
					&&stringBuffer.indexOf("if (strncmp(r->parsed_uri.path, include, strlen(include))) {")>0
					&&stringBuffer.indexOf("if (strncmp(r->parsed_uri.path, exclude, strlen(exclude))) {")>0
					) {
