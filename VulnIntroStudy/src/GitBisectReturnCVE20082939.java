 ...
 * 
 */
public class GitBisectReturnCVE20082939 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2008-2939>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("if (wildcard != NULL) {")>0
					&&stringBuffer.indexOf("wildcard = ap_escape_html(p, wildcard);")>0
					&&stringBuffer.indexOf("APR_BRIGADE_INSERT_TAIL(out, apr_bucket_pool_create(wildcard,")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("if (wildcard != NULL) {")>0
					&&stringBuffer.indexOf("wildcard = ap_escape_html(p, wildcard);")<0
					&&stringBuffer.indexOf("APR_BRIGADE_INSERT_TAIL(out, apr_bucket_pool_create(wildcard,")>0
					) {
