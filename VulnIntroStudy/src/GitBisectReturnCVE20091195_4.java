...
/**
 * CVE-2009-1195: /server/core.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20091195_4 {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("conf->override_opts = OPT_UNSET | OPT_ALL | OPT_SYM_OWNER | OPT_MULTI;")>0
					&&stringBuffer.indexOf("if ((base->opts & OPT_INC_WITH_EXEC)")>0
					&&stringBuffer.indexOf("&& (new->opts & OPT_INC_WITH_EXEC) == 0) {")>0
					&&stringBuffer.indexOf("conf->opts &= ~OPT_INC_WITH_EXEC;")>0
					&&stringBuffer.indexOf("opt = (OPT_INCLUDES | OPT_INC_WITH_EXEC);")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("conf->override_opts = OPT_UNSET | OPT_ALL | OPT_INCNOEXEC | OPT_SYM_OWNER")>0
					&&stringBuffer.indexOf("if ((base->opts & OPT_INCNOEXEC) && (new->opts & OPT_INCLUDES)) {")>0
					&&stringBuffer.indexOf("conf->opts = (conf->opts & ~OPT_INCNOEXEC) | OPT_INCLUDES;")>0
					&&stringBuffer.indexOf("opt = (OPT_INCLUDES | OPT_INCNOEXEC);")>0
					) {
