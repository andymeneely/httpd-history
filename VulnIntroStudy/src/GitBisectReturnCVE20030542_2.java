...
/**
 * CVE-2003-0542: /modules/filters/mod_include.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20030542_2 {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			...
			if(stringBuffer.indexOf("#define MAX_NMATCH 10")<0
					&&stringBuffer.indexOf("regmatch_t  match[AP_MAX_REG_MATCH];")>0
					&&stringBuffer.indexOf(" if (re->nsub < idx || idx >= AP_MAX_REG_MATCH) {")>0
					&&stringBuffer.indexOf("rc = !ap_regexec(compiled, string, AP_MAX_REG_MATCH, re->match, 0);")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("#define MAX_NMATCH 10")>0
					&&stringBuffer.indexOf("regmatch_t  match[MAX_NMATCH];")>0
					&&stringBuffer.indexOf("if (re->nsub < idx) {")>0
					&&stringBuffer.indexOf("rc = !ap_regexec(compiled, string, MAX_NMATCH, re->match, 0);")>0
					) {
