...
/**
 * CVE-2003-0542: /modules/metadata/mod_setenvif.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20030542_5 {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			...
			if(stringBuffer.indexOf("regmatch_t regm[AP_MAX_REG_MATCH];")>0
					&&stringBuffer.indexOf("(!b->pattern && !ap_regexec(b->preg, val, AP_MAX_REG_MATCH, regm,")>0
				...
				goodCommit = true;
					...
					&&stringBuffer.indexOf("(!b->pattern && !ap_regexec(b->preg, val, b->preg->re_nsub + 1,")>0
					&&stringBuffer.indexOf("b->preg->re_nsub + 1, regm);")>0
					) {
