 ...
 * 
 */
public class GitBisectReturnCVE20101623 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-201-1623>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("static apr_status_t brigade_append(apr_bucket_brigade *bbOut, apr_bucket_brigade *bbIn)")>0
					&&stringBuffer.indexOf("rv = brigade_append(ccfg->tmpbb, bb);")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("APR_BRIGADE_CONCAT(ccfg->tmpbb, bb);")>0
					/*&&stringBuffer.indexOf("static apr_status_t brigade_append(apr_bucket_brigade *bbOut, apr_bucket_brigade *bbIn)")<0
					&&stringBuffer.indexOf("rv = brigade_append(ccfg->tmpbb, bb);")<0*/
					) {
