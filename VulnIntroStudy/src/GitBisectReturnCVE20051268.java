...
/**
 * CVE-2005-1268: /modules/ssl/ssl_engine_kernel.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20051268 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2005-1268>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("BIO *bio = BIO_new(BIO_s_mem());")>0
					&&stringBuffer.indexOf("n = BIO_read(bio, buff, sizeof(buff) - 1);")>0
					&&stringBuffer.indexOf("n = BIO_read(bio, buff, sizeof(buff));")<0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("BIO *bio = BIO_new(BIO_s_mem());")>0
					&&stringBuffer.indexOf("n = BIO_read(bio, buff, sizeof(buff) - 1);")<0
					&&stringBuffer.indexOf("n = BIO_read(bio, buff, sizeof(buff));")>0
					) {
