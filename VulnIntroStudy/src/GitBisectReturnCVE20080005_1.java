...
/**
 * CVE-2008-0005: /modules/dav/main/mod_dav.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20080005_1 {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("static int dav_error_response(request_rec *r, int status, const char *body)")>0
					&&stringBuffer.indexOf("r->status = status;")>0
					...
					&&stringBuffer.indexOf("/* begin the response now... */")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("static int dav_error_response(request_rec *r, int status, const char *body)")>0
					&&stringBuffer.indexOf("r->status = status;")>0
					&&stringBuffer.indexOf("ap_set_content_type(r, \"text/html; charset=ISO-8859-1\");")<0
					&&stringBuffer.indexOf("ap_set_content_type(r, \"text/html\");")>0
					&&stringBuffer.indexOf("/* begin the response now... */")>0
