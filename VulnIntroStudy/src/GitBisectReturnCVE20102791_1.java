...
/**
 * CVE-2011-2791 : /modules/http/http_filters.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20102791_1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2011-2791>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("#include \"http_connection.h\"")>0 
					&&stringBuffer.indexOf("ap_bucket_error *eb = NULL;")>0
					&&stringBuffer.indexOf("if (AP_BUCKET_IS_ERROR(e) && !eb) {")>0
					&&stringBuffer.indexOf("* If we see an EOC bucket it is a signal that we should get out")>0
					&&stringBuffer.indexOf(" if (AP_BUCKET_IS_EOC(e)) {")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("#include \"http_connection.h\"")<0 
					&&stringBuffer.indexOf("ap_bucket_error *eb = e->data;")>0
					&&stringBuffer.indexOf("if (AP_BUCKET_IS_EOC(e)) {")<0 
					&& stringBuffer.indexOf("* If we see an EOC bucket it is a signal that we should get out")<0
					) {
