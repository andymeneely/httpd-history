...
/**
 * CVE-2004-0174: /server/listen.c avoiding configure.in as the fix was a completely new feature, independent
 * of the existing code and hence vulnerable since first commit
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20040174_4 {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("ap_listen_rec *ap_listeners = NULL;") > 0
					&& stringBuffer.indexOf("ap_listeners = new;") > 0
					&& stringBuffer.indexOf(" ap_listeners = last = new;") > 0
					&& stringBuffer.indexOf("ap_listeners = lr->next;") > 0
					&& stringBuffer.indexOf("#if AP_NONBLOCK_WHEN_MULTI_LISTEN") > 0
					&& stringBuffer.indexOf("if (ap_listeners->next) {") > 0
					&& stringBuffer.indexOf("for (lr = ap_listeners; lr; lr = lr->next) {") > 0
					&& stringBuffer.indexOf("status = apr_socket_opt_set(lr->sd, APR_SO_NONBLOCK, 1);") > 0
					&& stringBuffer.indexOf("#endif /* AP_NONBLOCK_WHEN_MULTI_LISTEN */") > 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("ap_listen_rec *ap_listeners = NULL;") > 0
					&& stringBuffer.indexOf("for (lr = ap_listeners; lr; lr = lr->next) {") > 0
					&& stringBuffer.indexOf("ap_listeners = new;") > 0
					&& stringBuffer.indexOf(" ap_listeners = last = new;") > 0
					&& stringBuffer.indexOf("ap_listeners = lr->next;") > 0
