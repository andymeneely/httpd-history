...
/**
 * CVE-2009-1195: /server/config.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20091195_3 {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("parms.override_opts = OPT_ALL | OPT_SYM_OWNER | OPT_MULTI;")>0
					&&stringBuffer.indexOf("parms.override_opts = OPT_ALL | OPT_INCNOEXEC | OPT_SYM_OWNER | OPT_MULTI;")<0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("parms.override_opts = OPT_ALL | OPT_INCNOEXEC | OPT_SYM_OWNER | OPT_MULTI;")>0
					&&stringBuffer.indexOf("parms.override_opts = OPT_ALL | OPT_SYM_OWNER | OPT_MULTI;")<0
					) {
