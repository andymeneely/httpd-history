...
/**
 * CVE-2010-0425: /arch/win32/mod_isapi.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20100425 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2010-0425>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("if (res) {") > 0 && stringBuffer.indexOf("isapi_unload(isa, 0);") < 0
					&& stringBuffer.indexOf("if (res < 0) {") > 0
					&& stringBuffer.indexOf("return HTTP_INTERNAL_SERVER_ERROR;") > 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("if (res) {") > 0 && stringBuffer.indexOf("isapi_unload(isa, 0);") > 0
					&& stringBuffer.indexOf("if (res < 0) {") > 0
					&& stringBuffer.indexOf("return HTTP_INTERNAL_SERVER_ERROR;") > 0) {
