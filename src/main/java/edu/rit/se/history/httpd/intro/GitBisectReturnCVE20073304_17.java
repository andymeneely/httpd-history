...
/**
 * CVE-2007-3304: /server/scoreboard.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20073304_17 {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("#include \"mpm_common.h\"")>0
					&&stringBuffer.indexOf("if (ap_scoreboard_image->parent[i].pid == pid->pid &&")>0
					&&stringBuffer.indexOf("ap_in_pid_table(pid->pid)) {")>0
					&&stringBuffer.indexOf("if (ap_scoreboard_image->parent[i].pid == pid->pid) {")<0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("#include \"mpm_common.h\"")<0
					&&stringBuffer.indexOf("if (ap_scoreboard_image->parent[i].pid == pid->pid &&")<0
					&&stringBuffer.indexOf("ap_in_pid_table(pid->pid)) {")<0
					&&stringBuffer.indexOf("find_child_by_pid(apr_proc_t *pid)")>0
					&&stringBuffer.indexOf("if (ap_scoreboard_image->parent[i].pid == pid->pid)")>0
					) {
