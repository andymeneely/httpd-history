...
/**
 * CVE-2007-3304: /server/mpm/experimental/event/event.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20073304_4 {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("#define MPM_NOTE_CHILD_KILLED(i) do {         \\") > 0
					&& stringBuffer.indexOf("ap_unset_pid_table(MPM_CHILD_PID(i)); \\") > 0
					&& stringBuffer.indexOf("MPM_CHILD_PID(i) = 0;") > 0 && stringBuffer.indexOf("} while(0)") > 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("#define MPM_NOTE_CHILD_KILLED(i) (MPM_CHILD_PID(i) = 0)") > 0
					&& stringBuffer.indexOf("#define MPM_NOTE_CHILD_KILLED(i) do {") < 0) {
