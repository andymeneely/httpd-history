...
/**
 * CVE-2006-3747: /modules/mappers/mod_rewrite.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20063747 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2006-3747>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("char *cp;") > 0 && stringBuffer.indexOf("cp = uri + scheme;") > 0
					&& stringBuffer.indexOf("while (*cp && c < 4) {") > 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("char *cp;") > 0 && stringBuffer.indexOf("cp = uri + scheme;") > 0
					&& stringBuffer.indexOf("while (*cp && c < 5) {") > 0) {
