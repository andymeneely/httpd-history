...
/**
 * CVE-2003-0542: /modules/mappers/mod_alias.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20030542_3 {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("regmatch_t regm[AP_MAX_REG_MATCH];") > 0
					&& stringBuffer.indexOf("if (!ap_regexec(p->regexp, r->uri, AP_MAX_REG_MATCH, regm, 0)) {") > 0
					&& stringBuffer.indexOf("AP_MAX_REG_MATCH, regm);") > 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("regmatch_t regm[10];") > 0
					&& stringBuffer.indexOf("if (!ap_regexec(p->regexp, r->uri, p->regexp->re_nsub + 1, regm,") > 0
					&& stringBuffer.indexOf("p->regexp->re_nsub + 1, regm);") > 0) {
