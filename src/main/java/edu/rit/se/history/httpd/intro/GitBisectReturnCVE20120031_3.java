...
/**
 * CVE-2012-0031 First instance where we see that a fix involved multiple files. _3 denotes bisect script for
 * the file: scoreboard.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20120031_3 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2012-0031>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("static ap_scoreboard_e scoreboard_type;") > 0
					&& stringBuffer.indexOf("if (scoreboard_type == SB_SHARED) {") > 0
					&& stringBuffer.indexOf("scoreboard_type = sb_type;") > 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("ap_scoreboard_image->global->sb_type = sb_type;") > 0
					&& stringBuffer.indexOf("if (ap_scoreboard_image->global->sb_type == SB_SHARED) {") > 0
					&& stringBuffer.indexOf(" static ap_scoreboard_e scoreboard_type;") < 0) {
