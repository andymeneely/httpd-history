...
/**
 * CVE-2010-1452: /modules/dav/main/util.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20101452_2 {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("/* 2518 specifies this must be an absolute URI; just take the") > 0
					&& stringBuffer.indexOf("if ((rv = apr_uri_parse(r->pool, uri, &parsed_uri)) != APR_SUCCESS") > 0
					&& stringBuffer.indexOf(" || !parsed_uri.path) {") > 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("/* 2518 specifies this must be an absolute URI; just take the") > 0
					&& stringBuffer.indexOf("if ((rv = apr_uri_parse(r->pool, uri, &parsed_uri)) != APR_SUCCESS) {") > 0
					&& stringBuffer.indexOf(" || !parsed_uri.path) {") < 0) {
