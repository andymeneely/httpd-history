...
/**
 * CVE-2011-3368 Revision 1172939 and file protocol.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20113368 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2011-3368>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("static int read_request_line(request_rec") > 0
					&& stringBuffer.indexOf("&& !r->parsed_uri.scheme") > 0
					&& stringBuffer.indexOf("&& !(uri[0] == '*' && uri[1] == '\\0')) {") > 0
					&& stringBuffer.indexOf("ap_log_rerror(APLOG_MARK, APLOG_INFO, 0, r, APLOGNO(00559)") > 0
					&& stringBuffer.indexOf("if (r->method_number != M_CONNECT") > 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("static int read_request_line(request_rec") > 0
					&& stringBuffer.indexOf("&& !(uri[0] == '*' && uri[1] == \'\0')) {") < 0
					&& stringBuffer.indexOf("&& !r->parsed_uri.scheme") < 0
					&& stringBuffer.indexOf("if (r->method_number != M_CONNECT") < 0
					&& stringBuffer.indexOf("ap_log_rerror(APLOG_MARK, APLOG_INFO, 0, r, APLOGNO(00559)") < 0) {
