 ...
 * 
 */
public class GitBisectReturnCVE20030083_4 {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("#define T_ESCAPE_LOGITEM      (0x10)") > 0
					&& stringBuffer.indexOf("\"#define T_ESCAPE_LOGITEM       (%u)\\n\"") > 0
					&& stringBuffer.indexOf("T_ESCAPE_LOGITEM);") > 0
					&& stringBuffer.indexOf("if (!apr_isprint(c) || c == '\"' || c == '\\' || apr_iscntrl(c)) {") > 0
					&& stringBuffer.indexOf("flags |= T_ESCAPE_LOGITEM;") > 0
					&& stringBuffer.indexOf("printf(\"%u%c\", flags, (c < 255) ? ',' : ' ');") > 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("printf(\"%u%c\", flags, (c < 255) ? ',' : ' ');") > 0
					&& stringBuffer.indexOf("T_HTTP_TOKEN_STOP);") > 0) {
