 ...
 * 
 */
public class GitBisectReturnCVE20076388 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2007-6388>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("#include \"apr_strings.h\"")>0
					&&stringBuffer.indexOf("case STAT_OPT_REFRESH: {")>0
					&&stringBuffer.indexOf("apr_size_t len = strlen(status_options[i].form_data_str);")>0
					&&stringBuffer.indexOf("if (*(loc + len ) == '=') {")>0
					&&stringBuffer.indexOf("apr_ltoa(r->pool, t < 1 ? 1 : t));")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("#include \"apr_strings.h\"")<0
					&&stringBuffer.indexOf("if (*(loc + strlen(status_options[i].form_data_str)) == '='")>0
					&&stringBuffer.indexOf("&& atol(loc + strlen(status_options[i].form_data_str)")>0
					&&stringBuffer.indexOf("strlen(status_options[i].hdr_out_str) +")>0
					) {
