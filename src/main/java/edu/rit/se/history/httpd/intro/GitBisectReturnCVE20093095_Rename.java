 ...
 * @author harsha
 * @version v2 for original vulnerability @see GitBisectReturnCVE20093095.java
 * 
 */
public class GitBisectReturnCVE20093095_Rename {
		...
		try {
			...
			if (stringBuffer.indexOf("if ((password = apr_table_get(r->headers_in, \"Authorization\")) != NULL") > 0
				...
				goodCommit = true;
					...
					&& stringBuffer.indexOf("&& strcasecmp(ap_getword(r->pool, &password, ' '), \"Basic\") == 0") > 0
					&& stringBuffer.indexOf("if (!ftp_check_string(password)) {") < 0
					&& stringBuffer.indexOf("else if ((user = r->parsed_uri.user) != NULL) {") > 0) {
