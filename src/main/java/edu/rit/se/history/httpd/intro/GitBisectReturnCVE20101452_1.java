...
/**
 * CVE-2010-1452: /modules/cache/cache_storage.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20101452_1 {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("path = r->uri;")>0
					&&stringBuffer.indexOf("\"cache: Key for entity %s?%s is %s\", r->uri,")>0
					&&stringBuffer.indexOf("* Check if we need to ignore session identifiers in the URL and do so")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("\"cache: Key for entity %s?%s is %s\", r->parsed_uri.path,")>0
					&&stringBuffer.indexOf("path = r->parsed_uri.path;")>0
					&&stringBuffer.indexOf("* Check if we need to ignore session identifiers in the URL and do so")>0
					) {
