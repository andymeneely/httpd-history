...
/**
 * CVE-2007-3304: /server/mpm/prefork/prefork.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20073304_13 {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("pid = ap_scoreboard_image->parent[n].pid;") > 0
					&& stringBuffer.indexOf("if (ap_scoreboard_image->servers[n][0].status != SERVER_DEAD) {") > 0
					&& stringBuffer.indexOf("ap_unset_pid_table(pid);") > 0
					&& stringBuffer.indexOf("ap_set_pid_table(pid);") > 0
					&& stringBuffer.indexOf("if (ap_in_pid_table(MPM_CHILD_PID(index))) {") > 0
					&& stringBuffer.indexOf("kill(MPM_CHILD_PID(index), AP_SIG_GRACEFUL);") > 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("if (ap_scoreboard_image->servers[n][0].status != SERVER_DEAD &&") > 0
					&& stringBuffer.indexOf("kill((pid = ap_scoreboard_image->parent[n].pid), 0) == -1) {") > 0
					&& stringBuffer.indexOf("ap_unset_pid_table(pid);") < 0
					&& stringBuffer.indexOf("ap_set_pid_table(pid);") < 0
					&& stringBuffer.indexOf("if (ap_in_pid_table(MPM_CHILD_PID(index))) {") < 0
					&& stringBuffer.indexOf("kill(ap_scoreboard_image->parent[index].pid, AP_SIG_GRACEFUL);") > 0) {
