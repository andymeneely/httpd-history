 ...
 * 
 */
public class GitBisectReturnCVE20030253_harsha {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2003-0253>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("if (APR_STATUS_IS_EINTR(status) && one_process && shutdown_pending) {") < 0
					&& stringBuffer.indexOf("status = listensocks[offset].accept_func(&csd,") > 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("if (APR_STATUS_IS_EINTR(status) && one_process && shutdown_pending) {") > 0
					&& stringBuffer.indexOf("status = listensocks[offset].accept_func(&csd,") > 0) {
