...
/**
 * CVE-2004-0488: /server/protocol.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20040488 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2004-0488>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("char buf1[MAX_STRING_LEN], buf2[MAX_STRING_LEN];") < 0
					&& stringBuffer.indexOf("auth_line = apr_pstrcat(r->pool, \"Basic \",") > 0
					&& stringBuffer.indexOf("ap_pbase64encode(r->pool,") > 0
					&& stringBuffer.indexOf("apr_pstrcat(r->pool, clientdn,") > 0
					&& stringBuffer.indexOf("\":password\", NULL)),") > 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("char buf1[MAX_STRING_LEN], buf2[MAX_STRING_LEN];") > 0
					&& stringBuffer.indexOf("apr_snprintf(buf1, sizeof(buf1), \"%s:password\", clientdn);") > 0
					&& stringBuffer.indexOf("ssl_util_uuencode(buf2, buf1, FALSE);") > 0
					&& stringBuffer.indexOf("apr_snprintf(buf1, sizeof(buf1), \"Basic %s\", buf2);") > 0
					&& stringBuffer.indexOf(" apr_table_set(r->headers_in, \"Authorization\", buf1);") > 0) {
