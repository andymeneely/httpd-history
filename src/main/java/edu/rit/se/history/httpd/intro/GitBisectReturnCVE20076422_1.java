...
/**
 * CVE-2007-6422: /modules/dav/main/util.c: Revision 607838
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20076422_1 {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("int set_etag = 0;")>0
					&&stringBuffer.indexOf("etag = apr_table_get(r->headers_out, \"ETag\");")>0
					&&stringBuffer.indexOf("apr_table_set(r->headers_out, \"ETag\", etag);")>0
					&&stringBuffer.indexOf("result = dav_meets_conditions(r, resource_state);")>0
					&&stringBuffer.indexOf("apr_table_unset(r->headers_out, \"ETag\");")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("int set_etag = 0;")<0
