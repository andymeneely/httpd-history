...
/**
 * CVE-2007-3304: /server/main.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20073304_2 {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("ap_pid_table               = apr_table_make(pglobal, 1024);") > 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("ap_pid_table               = apr_table_make(pglobal, 1024);") < 0) {
