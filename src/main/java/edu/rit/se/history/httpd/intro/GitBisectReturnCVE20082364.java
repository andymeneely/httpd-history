...
/**
 * CVE-2008-2364: /modules/proxy/mod_proxy_http.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20082364 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2008-2364>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName)
		...
		try {
			 ...
			 */
			if(stringBuffer.indexOf("#ifndef AP_MAX_INTERIM_RESPONSES")>0
					&&stringBuffer.indexOf("#define AP_MAX_INTERIM_RESPONSES 10")>0
					&&stringBuffer.indexOf("int interim_response = 0; /* non-zero whilst interim 1xx responses")>0
					&&stringBuffer.indexOf("if (ap_is_HTTP_INFO(r->status)) {")>0
					&&stringBuffer.indexOf("if (interim_response >= AP_MAX_INTERIM_RESPONSES) {")>0
					) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if(stringBuffer.indexOf("} while (interim_response);")>0
					&&stringBuffer.indexOf("interim_response = ap_is_HTTP_INFO(r->status);")>0
					&&stringBuffer.indexOf("int interim_response; /* non-zero whilst interim 1xx responses")>0
					) {
