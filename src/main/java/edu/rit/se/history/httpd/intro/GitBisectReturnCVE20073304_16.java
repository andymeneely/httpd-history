...
/**
 * CVE-2007-3304: /server/mpm_common.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20073304_16 {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("apr_table_t *ap_pid_table;") > 0
				...
				goodCommit = true;
			} else if (stringBuffer.indexOf("int ap_unregister_extra_mpm_process(pid_t pid)") > 0
					&& stringBuffer.indexOf("ap_unset_pid_table(pid);") < 0
					&& stringBuffer.indexOf("static int reclaim_one_pid(pid_t pid, action_t action)") > 0
					&& stringBuffer.indexOf("if (!ap_in_pid_table(pid)) {") < 0) {
