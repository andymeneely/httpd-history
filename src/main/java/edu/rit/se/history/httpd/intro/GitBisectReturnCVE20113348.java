...
/**
 * CVE-2011-3348 /modules/proxy/mod_proxy_ajp.c
 * @author harsha
 * @version v2
 * 
 */
public class GitBisectReturnCVE20113348 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Bisecting for <CVE-2011-3348>");
		try {
	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			 ...
			 */
			if (stringBuffer.indexOf("if (status == AJP_EOVERFLOW)") > 0
					&& stringBuffer.indexOf("else if  (status == AJP_EBAD_METHOD) {") > 0
					&& stringBuffer.indexOf("return HTTP_NOT_IMPLEMENTED;") > 0
					&& stringBuffer.indexOf("if (status == AJP_EOVERFLOW || status == AJP_EBAD_METHOD)") < 0) {
				System.out.println("Good Commit Context Met, commit was good");
				goodCommit = true;
			} else if (stringBuffer.indexOf("else if  (status == AJP_EBAD_METHOD) {") < 0
					&& stringBuffer.indexOf("return HTTP_NOT_IMPLEMENTED;") < 0
					&& stringBuffer.indexOf("if (status == AJP_EOVERFLOW || status == AJP_EBAD_METHOD)") > 0) {
