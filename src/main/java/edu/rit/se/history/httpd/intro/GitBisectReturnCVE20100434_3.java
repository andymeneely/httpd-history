	...
	public static boolean bisectBadOrGood(String fileName) throws FileNotFoundException {
		...
		try {
			...
			if (stringBuffer.indexOf("rnew->headers_in = apr_table_copy(rnew->pool, r->headers_in);") < 0
					...
					&& stringBuffer
							.indexOf("/* if a request with a body creates a subrequest, remove original request's") > 0
				...
				goodCommit = true;
					...
					.indexOf("/* if a request with a body creates a subrequest, clone the original request's") > 0
					&& stringBuffer
							.indexOf("* input headers minus any headers pertaining to the body which has already") > 0
